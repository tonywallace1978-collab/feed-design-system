...
**Last session:** Claude Code · 2026-04-26 (commit `ff52406`)
**Status:** Maria Lopez Professional Profile — 16 deviations vs canonical bundle cleared. renderReviews `wg` flag was reading non-existent `r.white_glove` field → always falsy → WG visual path was dead code → anonymous reviews rendered as broken "[A" placeholder tile. Fixed: derive `wg` from `r.company === '[Anonymous]'` (the actual signal in the data). Activates the canonical WG ◇ + amber treatment that #1's neutral-chrome fix already prepared. Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
