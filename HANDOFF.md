...
**Last session:** Claude Code · 2026-04-26 (commit `f836b44`)
**Status:** Maria Lopez Professional Profile — 23 deviations cleared. #23 stripped a 5th unauthorized field from Availability card (preference-flag chip row: OT OK / Weekends OK / No nights). Three-in-a-row scope-creep-with-privacy-leak pattern: #20 (Affiliation rate_independent), #22 (Education gpa+graduated_year), #23 (Availability willing_* preferences). All same shape: spec restricted the field set; renderer surfaced more; the extras leaked pre-negotiation posture. Bonus on #23: caught an asymmetric-negation bug — `if (!a.willing_nightshift)` defaults to surfacing the restriction, while the other two `willing_*` flags only render when true. Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
