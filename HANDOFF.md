...
**Last session:** Claude Code · 2026-04-26 (commit `2943a22`)
**Status:** Maria Lopez Professional Profile — 14 deviations vs canonical bundle cleared (#14 ships as #14a — the strictly-subtractive half; #14b is a product call for Tony, not a deviation — see "Open product calls" below). renderRadius stripped 5 fabricated city labels (Ann Arbor/Toledo/Lansing/Saginaw/Cleveland — none in D.location, same drift class as #4/#3/#12) and changed the hardcoded "Detroit" home-pin string to ${escapeHtml(L.city)}. Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
