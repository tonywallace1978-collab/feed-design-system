...
**Last session:** Claude Code · 2026-04-26 (commit `8b674db`)
**Status:** Maria Lopez Professional Profile — 20 deviations cleared. #20 stripped 2 unauthorized fields from the Affiliation card (`company_classification` chip + rate-row showing `rate_independent` $165/hr direct on a visitor-visible surface — same drift class as #15's PRIMARY chip but with an additional visibility-leak concern). Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
