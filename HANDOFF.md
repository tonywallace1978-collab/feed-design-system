...
**Last session:** Claude Code · 2026-04-26 (commit `9ad94b2`)
**Status:** Maria Lopez Professional Profile — 19 deviations cleared. **#19 is the highest-impact deviation flagged so far — trust-surface defect.** TÜV Functional Safety Certificate had `status: "uploaded"` + `expires_at: "2026-02-14"` (71 days past frozen clock 2026-04-26) — chipped UPLOADED green-ok, would have put a contractor on a job site without valid credentials. Renderer now derives expired state from `expires_at` vs today. Also: list now shows 10 rows per spec (was 13), status-prioritized sort so missing/expired bubble to the top (naive slice would have hidden the only `missing` row). Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
