...
**Last session:** Claude Code · 2026-04-26 (commit `ffb4025`)
**Status:** Maria Lopez Professional Profile — 24 deviations cleared. **PATTERN — four consecutive posture-leak deviations on left-rail [ALL] cards.** #20 (Affiliation rate_independent), #22 (Education gpa+grad_year), #23 (Availability willing_* flags), #24 (Rates salary band + "negotiable" suffix + engagement prefs). All same shape: spec author pruned posture-leak fields; renderer surfaced them back; rule-9 framing is the lens. #24 reverses much of my own #10 fix (which surfaced minimum_engagement_hours + rate_negotiable + currency-aware band) — at #10's time the archetype wasn't recognized; the lens shifted with #20+. Only one truly unaudited renderer remains: renderExperience. Strong recommendation to scan it for the same archetype as #25 before continuing one-at-a-time.
