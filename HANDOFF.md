...
**Last session:** Claude Code · 2026-04-26 (commit `23cc17c`)
**Status:** Maria Lopez Professional Profile — 22 deviations cleared. #22 stripped 4 unauthorized fields from Education card (city, graduated_year, gpa, honors). Same drift archetype as #20: surface scope-creep with privacy/posture leak as the urgency lift — `graduated_year` is the strongest age-discrimination data point on the page; `gpa` is academic ranking on a visitor-visible hiring surface. Spec author quietly pruned both by saying "degrees + schools" not "degrees + schools + when + grades." Spec-compliance pass for Maria Pro continues — Tony to eyeball before moving to next surface.
